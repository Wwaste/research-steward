...
Bind the Node service to loopback and publish it only through Tailscale Serve:

```bash
tailscale serve --bg --yes --set-path /research-steward http://127.0.0.1:8799
```

The remote MCP endpoint is then:

```text
https://<tailnet-dns-name>/research-steward/mcp
```

Keep the bearer token in the client's secret store or environment injection mechanism. `/healthz` proves only that the process and host allowlist are working; an authenticated MCP `tools/list` call is required to prove the full client path.

Run the repository's endpoint probe after promotion. Inject the token without placing it in shell history or command output:

```bash
cd plugins/research-steward
RESEARCH_STEWARD_ENDPOINT=https://<tailnet-dns-name>/research-steward/mcp \
...
5. Verify local health, tailnet health, unauthorized `401`, authenticated `tools/list`, and service logs.
...
- Other MCP clients: use the same bundled `dist/server.mjs` over stdio, or configure the authenticated tailnet URL for shared server projects.
